...
export default function App() {
  ...
  const handleAuthSubmit = async (e) => {
    e.preventDefault();
    try {
      const endpoint = authMode === 'signin' ? `${API_URL}/api/auth/login` : `${API_URL}/api/auth/signup`;
      ...
      const contentType = res.headers.get("content-type");
      if (!contentType || !contentType.includes("application/json")) {
        throw new Error("Backend server is not running or returned an invalid response format.");
      }
      ...
      if (data.user.status !== 'Approved' && data.user.role !== 'admin') {
        alert('Your account is currently pending administrator approval.');
        return;
      }

      setCurrentUserProfile(data.user);
      setCurrentView('dashboard');
      setDashboardTab(data.user.role === 'admin' ? 'admin' : 'overview');
    } catch (err) {
      console.error(err);
  ...
  const handleUpdateUserStatus = async (id, currentStatus) => {
    ...
    // Update local state immediately per specific user ID with robust fallback matching
  ...
  const handleUpdateUserField = async (id, field, value) => {
    ...
    if (!id) {
      console.error('Cannot update field: User ID is missing.');
      return;
    }

    // Update local state strictly for this unique user ID using a robust fallback matching id or _id
    setUsersList(prev => prev.map(u => {
      const userId = u.id || u._id;
      if (userId === id) {
        ...
        <div style={{ width: '100%', maxWidth: '440px', backgroundColor: '#111827', padding: '40px', borderRadius: '16px', border: '1px solid rgba(59, 130, 246, 0.3)', boxShadow: '0 20px 40px rgba(0,0,0,0.6)' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            ...
            <p style={{ fontSize: '13px', color: '#94A3B8' }}>
              {authMode === 'signin' ? 'Enter your credentials to access your dashboard' : 'Register for institutional portfolio management. Requires admin approval.'}
            </p>
          </div>
          ...
          <form onSubmit={handleAuthSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {authMode === 'signup' && (
              <>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: '#E2E8F0', marginBottom: '6px' }}>Phone Number</label>
                  <input 
                    type="text" 
                    required 
                    ...
                    {usersList.map((u) => {
                      ...
                      return (
                        <tr key={rowId} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                          ...
                          {/* Initial Deposit Input - Strictly bound to rowId */}
                          ...
                          {/* Interest Input - Strictly bound to rowId */}
                          <td style={{ padding: '16px' }}>
                            <input 
                              type="text" 
                              value={u.interest ?? ''} 
                          ...
                          {/* Total Balance Input - Strictly bound to rowId */}
                          <td style={{ padding: '16px' }}>
                            <input 
                              type="text" 
                              value={u.total ?? ''} 
        ...
        {mobileMenuOpen && (
          <div style={{ position: 'absolute', top: '100%', left: 0, width: '100%', backgroundColor: '#111827', borderBottom: '1px solid rgba(59, 130, 246, 0.3)', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px', zIndex: 50, boxSizing: 'border-box', boxShadow: '0 15px 25px rgba(0,0,0,0.7)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingTop: '12px', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
              <button onClick={() => { setMobileMenuOpen(false); setAuthMode('signin'); setCurrentView('auth'); }} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid rgba(37, 99, 235, 0.4)', color: '#FFFFFF', fontWeight: 600, background: 'transparent', cursor: 'pointer' }}>Sign In</button>
              <button onClick={() => { setMobileMenuOpen(false); setAuthMode('signup'); setCurrentView('auth'); }} style={{ width: '100%', padding: '12px', borderRadius: '10px', background: 'linear-gradient(to right, #2563EB, #1D4ED8)', color: '#FFFFFF', fontWeight: 600, border: 'none', cursor: 'pointer' }}>Get Started</button>
            </div>
          </div>
        )}
      </header>
                    ...
                    <>
                      <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                          <button onClick={() => { setAuthMode('signin'); setCurrentView('auth'); }} style={{ backgroundColor: 'rgba(37, 99, 235, 0.2)', color: '#38BDF8', border: '1px solid rgba(37, 99, 235, 0.4)', padding: '6px 16px', borderRadius: '8px', fontWeight: 700, cursor: 'pointer' }}>Trade</button>
                        </td>
                      </tr>
                      <tr>
